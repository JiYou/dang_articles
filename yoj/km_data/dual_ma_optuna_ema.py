...
def crossover(series1, series2):
    # 定义金叉：前一刻 series1 < series2，当前 series1 >= series2
    return (series1.shift(1) < series2.shift(1)) & (series1 >= series2)

def crossunder(series1, series2):
    # 定义死叉：前一刻 series1 > series2，当前 series1 <= series2
    return (series1.shift(1) > series2.shift(1)) & (series1 <= series2)
...
def strategy_backtest(df, length0, p1, p2, p3, return_extras=False):
    ...
    # 1. 计算指标
    # avg1.iloc[i] 的值依赖于 close.iloc[i]，即第i天的收盘价
    ...
    # 循环从第1行开始，因为第0行没有前一天的收盘价来计算浮动盈亏
    for i in range(1, len(df)):
        daily_pnl = 0.0
        # 1. 计算持仓浮动盈亏 (基于收盘价到收盘价的变化)
        if position == 1:
        ...
        # 2. 处理交易信号（次日开盘价成交）
        #    现在 buy_signal.iloc[i] 是基于 i-1 的信息，所以可以在 i 的开盘价交易
    ...
    if return_extras:
        # 注意：返回的avg均线序列没有移动，它们仍然是与价格对齐的，用于绘图
...
def main():
    ...
    print(f"Profit on training set (without future function): {in_sample_profit:.2f}")
    print("-" * 30)
    
    print("--- Out-of-Sample Results (Test Set) ---")
    print(f"Applying parameters {best_params} to the test set.")
    print(f"Profit on test set (without future function): {out_of_sample_profit:.2f}")
    print("-" * 30)
